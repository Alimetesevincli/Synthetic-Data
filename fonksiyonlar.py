...
def metrikler(model_type,df):
    NN_models={"Spearman":spearman_correlation}
    CC_models={ "chi-sq":chi_square_test}
...
def difference_p_value(df,sentetik):

    df_p_value=distinct(do_stats(df))
    sentetik_p_value=distinct(do_stats(sentetik))
    ...
    return df_p_value,df_p_value["Relative Difference"].mean(),df_p_value["Relative Difference"].median()
...
def sentetik_prod(df,size,seed,json_file_obj=None,_trained_model=None):#####DEĞİŞTİ#######
    ...
    if json_file_obj:
        columns_info = load_json(json_file_obj)
        validate_dataset_columns(df, columns_info)
    if _trained_model:
    ...
    gaussian.fit(df.sample(10000,random_state=seed))

    return gaussian.sample(size)
...
def normal_dist(df,datasets,column,max_value):
    
    df=df.drop(df.loc[df[column]>max_value].index)
    for name,data in datasets.items():
        
        # print(name,data)
        data=data.drop(data.loc[data[column]>max_value].index)
            

    fig, axes = plt.subplots(len(datasets),1,figsize=(10, 5))


    j=0
    for name,sets in datasets.items():
            
        sns.kdeplot(sets[column], label=f'{column}+{name}', fill=True, ax=axes[j])
        sns.kdeplot(df[column], label=f'{column}+{name}', fill=True, ax=axes[j])

        axes[j].set_title(f'Normal Dağılım Grafiği - {column}+{name}')
        axes[j].legend()
...
def cat_freq(data,sentetik,column_name,name):
    count1 = data[column_name].value_counts(normalize=True).reset_index()
    count1.columns = ['result', 'percentage']
    count1['dataset'] = 'Real Data'

    count2 = sentetik[column_name].value_counts(normalize=True).reset_index()
    count2.columns = ['result', 'percentage']
    count2["dataset"] = name+" Data"
    
    combined_counts = pd.concat([count1, count2])

    fig=plt.figure(figsize=(10, 6))
    sns.barplot(x='result', y='percentage', hue='dataset', data=combined_counts, palette='muted')

    plt.title('Categorical Percentage Frequency Comparison')
    plt.xlabel('Result')
